...
passedRES = True
passedHV = True
daq = None #to control the daq
kei = None #to control the keithley
    ...
    while (timeout):
        ...
        if (res is not None):
            if ( (lowerLimit is None or lowerLimit < res) and (upperLimit is None or res < upperLimit) ):
                ...
                return res
        time.sleep(0.1)
        #does it apply voltage when we close switch or when we measure res?
        timeout -= 1
        print("DEBUG: ___ timeout: {} res: {}".format(timeout, res))
...
def testCurrent(start, end):
    ...
    for v in range(start, end + step, step): # Sweeps from current voltage to desired voltage
